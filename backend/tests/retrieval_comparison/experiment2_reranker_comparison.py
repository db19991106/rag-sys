"""
实验二：无精排 vs BGE Reranker精排准确率对比
"""
...
TOP_K = 5
CANDIDATE_K = 20
OUTPUT_PATH = project_root.parent / "test_reports" / "使用二次精排准确率对比.md"


def run_no_rerank_experiment(ground_truth: Dict) -> Dict[str, Any]:
    """
    小实验1：无精排评估
...
def run_rerank_experiment(ground_truth: Dict) -> Dict[str, Any]:
    """
    小实验2：BGE Reranker精排评估
    ...
    config = {
        ...
        'enable_rerank': True,
        'reranker_type': 'bge',
        'reranker_model': '/root/autodl-tmp/models/bge-reranker-large'
    }
    ...
    # 初始化Reranker
    from services.reranker import RerankerManager
    
    reranker_manager = RerankerManager()
    reranker_manager.initialize(
        reranker_type='bge',
        model_name='/root/autodl-tmp/models/bge-reranker-large',
        device='cuda',
        top_k=TOP_K,
        threshold=0.0
    )
    ...
    for i, (query_id, data) in enumerate(ground_truth.items()):
        ...
        try:
            ...
            # 5. 执行Reranker精排
            # rerank()返回[(原始索引, 分数), ...]，已按分数降序排列
            rerank_results = reranker_manager.reranker.rerank(query_text, candidate_contents)
            
            # 6. 根据Reranker结果获取Top-K（使用原始索引获取对应的chunk和content）
            retrieved_chunks = []
            retrieved_contents = []
            for original_idx, score in rerank_results[:TOP_K]:
                retrieved_chunks.append(candidate_chunks[original_idx])
                retrieved_contents.append(candidate_contents[original_idx])
    ...
    # 清理资源
    del vector_db_manager, bm25, reranker_manager
    clear_gpu_memory()
...
def main():
    ...
    # 小实验1：无精排
    no_rerank_metrics = run_no_rerank_experiment(ground_truth)
    
    # 等待资源释放
    wait_for_memory(5)
    
    # 小实验2：BGE Reranker精排
    rerank_metrics = run_rerank_experiment(ground_truth)
    ...
    generate_comparison_report(
        experiment_name="使用二次精排准确率对比",
        baseline_name="无精排（RRF融合）",
        optimized_name="BGE Reranker精排",
        baseline_metrics=no_rerank_metrics,
        optimized_metrics=rerank_metrics,
