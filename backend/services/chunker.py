...
class RAGFlowChunker:
    ...
    def __init__(self):
        # 章标题：支持 "## 第一章 总则" 或 "##第一章..."
        self.chapter_pattern = re.compile(r'^#{2}\s*(第[零一二三四五六七八九十百0-9]+章\s*.+)$', re.MULTILINE)
        # 节标题：支持 "### 2.5 培训与会议" 或 "###2.5培训..."（数字后可无空格）
        self.section_pattern = re.compile(r'^#{3}\s*([0-9]+\.[0-9]+\s*.+)$', re.MULTILINE)
        # 文档标题
    ...
    def chunk(self, content: str, doc_id: str, config: ChunkConfig) -> List[ChunkInfo]:
        ...
        try:
            ...
            # 智能切分：对于INTELLIGENT类型，无论是否是制度类文档，都使用财务/制度类文档的切分策略
            # 这样可以确保智能切分与PDF切分使用相同的逻辑
            if config.type == ChunkType.INTELLIGENT or (is_policy_doc and config.type in [ChunkType.PDF, ChunkType.ENHANCED]):
                logger.info(f"使用智能切分策略 (config.type: {config.type})。文档长度: {len(content)}")
    ...
    def _detect_policy_document(self, content: str) -> bool:
        """检测是否为制度类文档"""
        policy_keywords = ['报销', '审批', '第一章', '第.*章', '差旅费', '发票', '借款', '违规处理']
    ...
    def _analyze_chapter_structure(self, content: str) -> List[Dict]:
        ...
        logger.info(f"文档结构分析：识别到文档标题: {len([s for s in structure if s['type'] == 'doc_title'])} 个")
        logger.info(f"文档结构分析：识别到章: {len(chapters)} 个 - {[c['title'] for c in chapters]}")
        logger.info(f"文档结构分析：识别到节: {len(sections)} 个 - {[s['title'] for s in sections[:10]]}{'...' if len(sections) > 10 else ''}")
        
        if not chapters:
            logger.error("未识别到任何章节（第X章），请检查文档格式是否为Markdown（# ## ###）")
    ...
    def _financial_policy_chunking(self, content: str) -> List[Dict[str, Any]]:
        ...
        for match in self.table_pattern.finditer(content):
            ...
            content_with_placeholders = content_with_placeholders.replace(match.group(1), f"\n{placeholder}\n", 1)
        ...
        if not structure:
            ...
            return [{'content': content, 'type': 'doc', 'metadata': {'type': 'full_doc'}}]
        ...
        for idx, (start_pos, end_pos, chunk_type, metadata) in enumerate(boundaries, 1):
            if start_pos >= end_pos:
                logger.warning(f"片段 {idx} 的边界无效（{start_pos} >= {end_pos}），跳过")
                continue
            ...
            for table_info in protected_tables:
                if table_info['placeholder'] in chunk_content:
                    chunk_content = chunk_content.replace(table_info['placeholder'], table_info['content'])
    ...
    def _build_chunk_boundaries(self, structure: List[Dict], content: str) -> List[Tuple]:
        """
        ...
        if not chapters:
            ...
            return [(start, content_len, 'doc', {'hierarchy_path': [doc_title['title']] if doc_title else ['文档']})]
        ...
        if len(first_ch_sections) >= 2:
            # 包含前2节（通常是1.1和1.2）
            end_pos = first_ch_sections[2]['pos'] if len(first_ch_sections) > 2 else first_ch_end
        elif len(first_ch_sections) == 1:
            end_pos = first_ch_sections[0]['end'] + len(first_ch_sections[0]['title']) * 10  # 估算长度
        ...
        boundaries.append((
            ...
            'chapter', 
            {
                'hierarchy_path': [doc_title['title'] if doc_title else '文档', first_ch['title']],
                'chapter_index': 1
            }
        ))
        ...
        for i, ch in enumerate(chapters[1:], 2):
            ...
            has_table_in_sec1 = '__TABLE_' in chunk1_content or '|' in chunk1_content[:100]
            ...
            for j in range(sec_start_idx, len(ch_sections)):
                sec = ch_sections[j]
                next_pos = ch_sections[j+1]['pos'] if j+1 < len(ch_sections) else ch_end
                ...
                if j == len(ch_sections) - 1 and boundaries:
                    ...
                    if sec_len < 200 and boundaries[-1][1] == sec['pos']:
                        ...
                        if 'section_range' in last_boundary[3]:
                            ...
                            last_boundary[3]['section_range'] = f"{last_boundary[3].get('section_title', '')} + {sec['title']}"
        ...
        if boundaries and boundaries[-1][1] < content_len:
            ...
            if remaining_content and remaining_content != '---':  # 忽略分隔线
                logger.warning(f"发现未覆盖的文档尾部（{content_len - last_end} 字符），自动添加为最后一个片段")
                # 扩展最后一个片段到文档末尾
    ...
    def _create_chunk_infos(self, chunks_data: List[Dict], doc_id: str) -> List[ChunkInfo]:
        ...
        for i, chunk_data in enumerate(chunks_data, 1):
            ...
            logger.info(f"片段 {i:02d} | 长度: {chunk_info.length:4d} | 类型: {metadata.get('type', 'text'):15s} | 层级: {'/'.join(metadata.get('hierarchy_path', []))[:30]}...")
        
        return chunk_infos

    def _save_chunks_to_json(self, chunks: List[ChunkInfo], doc_id: str, config: ChunkConfig):
    ...
    def _naive_merge(self, content: str, chunk_token_num: int = 512, 
