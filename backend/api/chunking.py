...
async def split_document(doc_id: str, config: ChunkConfig):
    """
    切分文档
...
async def get_document_chunks(doc_id: str):
    ...
    try:
        ...
        if not chunks_file.exists():
            raise HTTPException(status_code=404, detail="未找到该文档的切分片段，请先切分文档")
        
        with open(chunks_file, 'r', encoding='utf-8') as f:
            chunks_data = json.load(f)
            chunks = [ChunkInfo(**chunk_data) for chunk_data in chunks_data.get("chunks", [])]
...
async def embed_chunks(doc_id: str):
    ...
    try:
        ...
        if doc.status != "split" and doc.chunk_count == 0:
            raise HTTPException(
                status_code=400,
                detail="文档未切分，请先调用 /chunking/split 切分文档"
            )
        ...
        if chunks_file.exists():
            ...
            with open(chunks_file, 'r', encoding='utf-8') as f:
                ...
                chunks = [ChunkInfo(**chunk_data) for chunk_data in chunks_data.get("chunks", [])]
            logger.info(f"成功加载 {len(chunks)} 个片段")
        else:
            # 如果文件不存在，需要重新切分
        ...
        return ApiResponse(
            success=True,
            message=f"成功向量化 {len(chunks)} 个片段并存储到向量数据库"
        )
