...
from models import DocumentUploadResponse, DocumentInfo, ApiResponse, ErrorResponse
...
def validate_file_upload(file: UploadFile) -> None:
    ...
    if len(file.filename) > 255:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="文件名过长（最大255字符）"
        )
...
async def get_delete_confirmation_token(doc_id: str):
    ...
    try:
        ...
        token = confirmation_manager.generate_token(
            operation="delete", resource_id=doc_id
        )

        return {"confirmation_token": token, "expires_in_minutes": 10}
    except HTTPException:
        raise
    except Exception as e:
...
async def delete_document(
    doc_id: str, confirmation_token: str = "", request: DeleteConfirmationRequest = None
):
    """
    删除文档（可选确认令牌）

    默认情况下不需要确认令牌即可删除。如果需要启用防误删保护，
    可以先调用 /{doc_id}/delete-confirmation-token 获取确认令牌。

    支持两种方式传递确认令牌：
    1. 查询参数：?confirmation_token=xxx
    2. 请求体：{"confirmation_token": "xxx"}
    """
    try:
        # 获取确认令牌（优先从查询参数，其次从请求体）
        token = confirmation_token
        if not token and request:
            token = request.confirmation_token

        # 如果提供了有效令牌，验证它；如果为空或None，跳过验证
        if token and token.strip():
            if not confirmation_manager.verify_token(
                token=token, operation="delete", resource_id=doc_id
            ):
                raise HTTPException(
        ...
        log_sensitive_operation(
            operation="delete_document", resource_id=doc_id, success=True
        )
...
async def get_batch_delete_confirmation_token(doc_ids: List[str]):
    ...
    try:
        ...
        token = confirmation_manager.generate_token(
            operation="batch_delete", resource_id=resource_id
        )
...
async def batch_delete_documents(request: Request):
    ...
    支持两种请求格式：
    1. 标准格式：{"doc_ids": [...], "confirmation_token": "..."}
    2. 兼容格式：直接发送文档ID数组（需配合简化版批量删除）
    """
    try:
        # 获取原始请求体
        ...
        if isinstance(body, list):
            ...
            raise HTTPException(status_code=400, detail="请求格式错误，应为数组或对象")
        ...
        return ApiResponse(success=True, message=f"成功删除 {deleted_count} 个文档")
    except HTTPException:
        raise
    except Exception as e:
        ...
        if doc.chunk_count > 0 and len(actual_chunks) == doc.chunk_count:
            ...
            if doc.chunk_count == 0 and len(actual_chunks) > 0:
                checks["details"].append(
                    f"! 切分数量记录为0，但实际有 {len(actual_chunks)} 个片段"
                )
            elif doc.chunk_count > 0:
                checks["details"].append(
                    f"✗ 切分数量不匹配: 记录={doc.chunk_count}, 实际={len(actual_chunks)}"
                )
            checks["overall_status"] = "warning"
            ...
            if doc_vector_count > 0:
                ...
                if doc.chunk_count > 0:
                    if doc_vector_count == doc.chunk_count:
                        ...
                        checks["details"].append(
                            f"✗ 向量数量不匹配: 记录={doc.chunk_count}, 实际={doc_vector_count}"
                        )
                        checks["overall_status"] = "warning"
                    ...
                    if doc_vector_count == len(actual_chunks):
                        ...
                        checks["details"].append(
                            f"✗ 向量数量不匹配: 片段={len(actual_chunks)}, 向量={doc_vector_count}"
                        )
                        checks["overall_status"] = "warning"
            else:
                checks["details"].append("✗ 向量不存在")
                checks["overall_status"] = (
                    "warning" if doc.status == "indexed" else "pending"
                )
        else:
